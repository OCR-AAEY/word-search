...
void open_file(GtkWidget *widget, gpointer label, GtkImage* image) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        gtk_label_set_text(GTK_LABEL(label), filename); // Update the label with the filename
	g_print("%s\n",filename);
...
void buttons_creation(GtkWidget** solve_b, GtkWidget** step_b, GtkWidget** exit_b)
{
...
void buttons_func(GtkWidget** solve_b, GtkWidget** step_b, GtkWidget** exit_b, GtkImage* image,GtkWidget ** window)
{
...
//    GtkWidget *img_cont;
    ...
    box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 10); // 10px spacing between elements
    ...
    gtk_box_pack_start(GTK_BOX(box), GTK_WIDGET(image), FALSE, FALSE, 10); // 10px spacing

    // Create a box to hold the buttons (vertically)
    button_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 10);  // 10px spacing between buttons 
    ...
    gtk_widget_set_valign(button_box, GTK_ALIGN_CENTER);  // Align the button box vertically in the center

    // Add the button box to the main box
    gtk_box_pack_end(GTK_BOX(box), button_box, TRUE, TRUE, 10);  // Add at the bottom
