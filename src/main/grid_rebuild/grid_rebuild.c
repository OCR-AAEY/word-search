...
static void build_cell_path(char *out, size_t outsz, const char *folder, size_t r, size_t c)
{
    if (!folder) { out[0] = '\0'; return; }
...
char recognize_letter_from_png(const char *path, Neural_Network *net)
{
    ...
    for (size_t cand = 1; cand * cand <= net_in; ++cand) {
        if (cand * cand == net_in) { s = cand; break; }
    }
    ...
    if (best < 26) return (char)('A' + (int)best);
    return '?';
}
 ...
 */
Grid *grid_rebuild_from_folder_with_model(const char *folder, size_t rows, size_t cols,
                                          const char *model_path)
{
    if (!folder || !model_path || rows == 0 || cols == 0) return NULL;
